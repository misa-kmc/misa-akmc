...
void placeDumbbells(LatticesList *lattice_list, ItlList *itl_list,
                    counter &m_counter, const LatticeTypes::lat_type types[],
...
int main() {
  r::initSeed(
      0x146221); // initialize random number seed // todo generate seed here

  // set configuration
  env::global_env =
      env::environment{6E12, // attempt_freq 6*10^12 // todo too big
                       773,  // temperature, K
                       3.53, // ef110
  ...
  sim_box->lattice_list->forAllLattices([&](const _type_lattice_coord x,
    ...
    const LatticeTypes::lat_type src_types[] = {
        LatticeTypes::Fe, LatticeTypes::Cu, LatticeTypes::Ni, LatticeTypes::Mn};
  ...
  const LatticeTypes::lat_type types[] = {LatticeTypes::Cu, LatticeTypes::Ni,
                                          LatticeTypes::Mn};
  unsigned long counts[] = {sim_box->lattice_list->getLatCount() * 150 / 10000,
                            0, 0}; // Cu 1.5%
  placeDumbbells(sim_box->lattice_list, sim_box->itl_list, m_counter, types,
                 counts, 3);
  ...
  // start simulation
  ABVIModel kmc{sim_box, env::global_env.attempt_freq,
                env::global_env.temperature}; // fixme init box pointer

  double current_time = 0;
  const double total_time = 0.1;
  unsigned long step = 0;
  const clock_t app_start_time = clock();
  const comm::Region<comm::_type_lattice_size> box_region{0,     0,     0,
                                                          box_x, box_y, box_z};
  while (current_time < total_time) {
    const _type_rate total_rates = kmc.calcRates(box_region);
    const event::SelectedEvent event =
        kmc.select(box_region, r::random() * total_rates, total_rates);
    ...
    step++;
    std::cout << "total rate: " << total_rates << "; time: " << current_time
              << std::endl;
    if (step % 10 == 0) {
      const double app_time =
          static_cast<double>(clock() - app_start_time) / CLOCKS_PER_SEC;
      std::cout << "program execution time (s):" << app_time << std::endl;
      std::cout << "==== KMC lattice count at time step " << step << ":"
                << std::endl;
      logCounter(m_counter);
    }
  }
...
void placeDumbbells(LatticesList *lattice_list, ItlList *itl_list,
  ...
  for (int tp = 0; tp < len; tp++) {
    ...
    for (int con = 0; con < count[tp]; con++) {
      const uint32_t id =
          r::rand32(0, static_cast<const uint32_t>(lattices_count));
