...
TEST(creation_setGlobalId_allreduce_test, creation_setGlobalId_test) {
  ...
  creation::setGlobalId(&lattice_list, p_domain->local_sub_box_lattice_region,
                        ...
                        {p_domain->phase_space[0], p_domain->phase_space[1],
                         p_domain->phase_space[2]});

  // sum ids of lattices in simulation box on current process
  comm::Region<comm::_type_lattice_coord> lbr =
  ...
  MPI_Allreduce(&id_local_sum, &id_sum, 1, MPI_UNSIGNED_LONG, MPI_SUM,
                MPI_COMM_WORLD);
  // test sum of all global ids
  EXPECT_EQ(id_sum, BCC_DBX * space[0] * space[1] * space[2] *
                        (BCC_DBX * space[0] * space[1] * space[2] - 1) / 2);
