...
TEST(lattices_list_getId_test, lattices_list) {
  ...
  auto *p_lat_list =
      new PeriodLatticeList(LatListMeta{box_x, box_y, box_z, g_x, g_y, g_z});
 ...
 */
inline _type_lattice_id ID_BOX_WITH_GHOST(LatListMeta meta,
                                          comm::_type_lattice_coord x,
...
TEST(get2nn_isGhostLat_low_boundary, lattice_list_ghost_test) {
  Lattice *_2nn[6] = {nullptr, nullptr, nullptr, nullptr,
                      nullptr, nullptr}; // a lattice has at most 6 2nn(s).
  PeriodLatticeList lattice_list(LatListMeta{4, 4, 4, 8, 8, 8});
  auto count = lattice_list.get2nn(lattice_list.meta.ghost_x + 0,
...
TEST(lattice_list_ghost_MaxId_test, lattice_list_ghost_test) {
  ...
  auto id =
      lattice_list.getId(2 * (4 + 8 + 8) - 1, 4 + 8 + 8 - 1, 4 + 8 + 8 - 1);
...
TEST(lattice_list_ghost_walk_test, lattice_list_ghost_test) {
  ...
  // case 1
  Lattice *lat =
      lattice_list.walk(ID_BOX_WITH_GHOST(lattice_list.meta, 4, 1, 1), 1, 1, 1);
  EXPECT_NE(lat, nullptr);
  EXPECT_EQ(lat->getId(), ID_BOX_WITH_GHOST(lattice_list.meta, 5, 1, 1));

  Lattice *lat2 =
      lattice_list.walk(ID_BOX_WITH_GHOST(lattice_list.meta, 5, 1, 1), 2, 2, 2);
  EXPECT_NE(lat2, nullptr);
  EXPECT_EQ(lat2, &(lattice_list.getLat(
                      ID_BOX_WITH_GHOST(lattice_list.meta, 7, 2, 2))));

  // invalid input
  Lattice *lat3 =
      lattice_list.walk(ID_BOX_WITH_GHOST(lattice_list.meta, 4, 1, 1), 1, 1, 2);
  EXPECT_EQ(lat3, nullptr);

  // invalid input
  Lattice *lat4 =
      lattice_list.walk(ID_BOX_WITH_GHOST(lattice_list.meta, 4, 1, 1), 2, 1, 2);
  EXPECT_EQ(lat4, nullptr);

  // test out of boundary(down).
  Lattice *lat5 = lattice_list.walk(
      ID_BOX_WITH_GHOST(lattice_list.meta, 4, 1, 1),
      -lattice_list.meta.ghost_x - 1, -2 * lattice_list.meta.ghost_y - 3,
