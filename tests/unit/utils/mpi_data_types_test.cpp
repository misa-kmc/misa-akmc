...
TEST(set_set_mpi_data_type_attice_test, set_mpi_data_type_test) {
  ...
  const kiwi::RID target_rank = (kiwi::mpiUtils::global_process.own_rank + 1) %
                                kiwi::mpiUtils::global_process.all_ranks;
  const kiwi::RID source_rank =
      (kiwi::mpiUtils::global_process.own_rank +
       kiwi::mpiUtils::global_process.all_ranks - 1) %
  ...
  MPI_Status status;
  MPI_Sendrecv(lattice_send, 5, lat_mpi_data_type, target_rank, 0x200,
               lattice_recv, 5, lat_mpi_data_type, source_rank, 0x200,
               MPI_COMM_WORLD, &status);

  for (int i = 0; i < 5; i++) {
    EXPECT_EQ(lattice_recv[i].type._type, LatticeTypes::FeCu);
    EXPECT_EQ(lattice_recv[i].id,
              1000 + i * kiwi::mpiUtils::global_process.own_rank);
  }
