...
TEST(bonds_counter_count_test, bonds_counter_test) {
  ...
  // case 1
  auto energy1 =
      BondsCounterTester::count(&lattice_list, lattice_list.getId(4, 2, 2),
                                LatticeTypes{LatticeTypes::Fe});
  EXPECT_EQ(energy1,
            BondsCounterTester::_1nn_bonds.at(bonds::PairBond::FeFe) * 8 +
                BondsCounterTester::_2nn_bonds.at(bonds::PairBond::FeFe) * 6);

  // case 2
  auto energy2 =
      BondsCounterTester::count(&lattice_list, lattice_list.getId(0, 0, 0),
                                LatticeTypes{LatticeTypes::Fe});
  EXPECT_EQ(energy2,
            BondsCounterTester::_1nn_bonds.at(bonds::PairBond::FeFe) * 1 +
...
TEST(bonds_counter_count_test_with_V, bonds_counter_test) {
  ...
  // rate for src
  const double rate_src =
      bonds::BondsCounter::count(&lattice_list, src_lat.getId(), src_lat.type);

  const double excepted_rate_src =
  ...
  // rate for target/tar
  const double rate_tar =
      bonds::BondsCounter::count(&lattice_list, tar_lat.getId(), tar_lat.type);

  const double excepted_rate_tar =
...
TEST(bonds_counter_count_test_with_V_2, bonds_counter_test) {
  ...
  const double excepted_rate_src =
      BondsCounterTester::_1nn_bonds.at(bonds::PairBond::FeFe) * 6 +
      BondsCounterTester::_1nn_bonds.at(bonds::PairBond::VFe) +
