...
bool PKMC::beforeCreate(int argc, char **argv) {
  ...
  args::ValueFlag<std::string> conf(parser, "conf", "The configure file",
                                    {'c', "conf"});
...
void PKMC::onCreate() {
  ...
  if (kiwi::mpiUtils::global_process.own_rank == MASTER_PROCESSOR) {
    ...
    p_config = ConfigParsing::newInstance(
        configFilePath); // todo config file from argv.
  ...
  p_config
      ->sync(); // sync config data to other processors from master processor.
...
bool PKMC::prepare() {
  ...
  case conf::None:
    ...
    return false;
  case conf::Random:
    creation::createRandom(
        config_v.seeds.create_types, config_v.seeds.create_vacancy,
        sim->box->lattice_list, sim->box->va_list, config_v.create.types,
        config_v.create.types_ratio, config_v.create.va_count, sim->_p_domain);
    break;
  case conf::Pipe:
    break;
  case conf::Restart:
...
void PKMC::onStart() {
  ...
  counter m_counter =
      counter::newCounter(sim->box->lattice_list); // atoms counter
  ...
  sim->simulate(&model, &m_event_hook, config_v.seeds.time_inc,
                config_v.physics_time);
}
