...
MEventHook::MEventHook(const conf::Output &output_conf,
                       LatticesList *p_lattice_list, counter *p_counter)
    : output_config(output_conf), p_lattice_list(p_lattice_list),
      p_counter(p_counter) {}
...
void MEventHook::onStepFinished(unsigned long step) {
  ...
  if (step % output_config.dump_interval == 0) {
    std::string dump_file_path =
        fmt::format(output_config.dump_file_path, step);
    const kiwi::mpi_process process = SimulationDomain::kiwi_sim_pro;
    if (process.all_ranks != 1) {
      dump_file_path = fmt::format(output_config.dump_file_path + ".{}", step,
                                   process.own_rank);
    }
