...
void MEventListener::onRecombine(const unsigned long time_step,
                                 const rec::Rec rec) {
  m_counter.subtract(LatticeTypes::V);
  m_counter.subtract(
      LatticeTypes::combineToInter(rec.tp_vac._type, rec.tp_inter._type));
  m_counter.add(rec.tp_vac._type);
  m_counter.add(rec.tp_inter._type);
}

void MEventListener::onDefectGenerate(
    const unsigned long time_step, LatticeTypes lat_1_type,
    LatticeTypes lat_2_type, const LatticeTypes::lat_type combined_type) {
