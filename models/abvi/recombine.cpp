...
rec::Rec rec::RecList::pickMinimum() {
    ...
    std::sort(rec_list.begin(), rec_list.end(), [](const Rec &a, const Rec &b) -> bool {
        return a.delta_energy < b.delta_energy;
    });
...
void rec::RecList::stageCombination(LatticesList *lat_list, ItlList *itl_list,
                                    Lattice *lat_vac, Lattice *lat_inter) {
    ...
    double e_before_h = 0, e_before_l = 0;
    {
        ...
        bonds::_type_pair_ia e_vac = bonds::BondsCounter::count(lat_list,
                                                                lat_vac->getId(),
                                                                lat_vac->type);
        // bonds energy of interstitial lattice (higher atom) contributed by its 1nn/2nn neighbour lattice.
        bonds::_type_pair_ia e_inter_h = bonds::BondsCounter::count(lat_list,
                                                                    lat_inter->getId(),
                                                                    LatticeTypes{type_inter.getHighEnd()});
        bonds::_type_pair_ia e_inter_l = e_inter_h;
        if (!same_high_low) {
            e_inter_l = bonds::BondsCounter::count(lat_list,
                                                   lat_inter->getId(),
                                                   LatticeTypes{type_inter.getLowEnd()});
        }
