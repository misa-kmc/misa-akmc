...
void SubLattice::startTimeLoop(Ins pk_inst, ModelAdapter<E> *p_model,
  ...
  for (unsigned long step = 0; step < time_steps; step++) { // time steps loop
    for (int sect = 0; sect < SECTORS_NUM; sect++) {        // sector loop
      const double step_threshold_time = static_cast<double>(step + 1) * T -
                                         sec_meta.sector_itl->evolution_time;
      double sector_time = 0.0;
      p_model->reindex(
          p_domain->local_sector_region[(*sec_meta.sector_itl)
                                            .id]); // reindex defects lists
      while (sector_time < step_threshold_time) {  // note: step_threshold_time
                                                  // may be less then 0.0
        const double total_rates =
            calcRatesWrapper(p_model, (*sec_meta.sector_itl).id);
        if (total_rates == 0.0 ||
            std::abs(total_rates) <
                std::numeric_limits<_type_rate>::epsilon()) {
      ...
      syncNextSectorGhostRegions<PKg>(
          pk_inst);          // communicate ghost area data of next sector in
                             // current process.
...
template <typename E>
double SubLattice::calcRatesWrapper(ModelAdapter<E> *p_model,
                                    const type_sector_id sector_id) {
  return p_model->calcRates(p_domain->local_sector_region[sector_id]);
}

template <typename E>
void SubLattice::selectPerformWrapper(ModelAdapter<E> *p_model,
                                      const _type_rate total_rates,
                                      const type_sector_id sector_id) {
  p_model->selectAndPerform(p_domain->local_sector_region[sector_id],
                            total_rates);
}
...
template <class PKs, class Ins> void SubLattice::syncSimRegions(Ins &pk_inst) {
  ...
  comm::singleSideForwardComm<typename PKs::pack_date_type,
                              typename PKs::pack_region_type, true>(
      &packer, SimulationDomain::comm_sim_pro, packer.getMPI_DataType(),
...
void SubLattice::syncNextSectorGhostRegions(Ins &pk_inst) {
  ...
  // pack data
  const std::array<unsigned int, comm::DIMENSION_SIZE> send_dirs =
      ssfdCommSendDirs(next_sector.id);
  const std::array<unsigned int, comm::DIMENSION_SIZE> recv_dirs =
      ssfdCommRecvDirs(next_sector.id);
  const std::array<unsigned int, comm::DIMENSION_SIZE> ranks_send = {
      static_cast<unsigned int>(
          p_domain->rank_id_neighbours[comm::DIM_X][send_dirs[0]]),
      static_cast<unsigned int>(
  ...
  comm::singleSideForwardComm(&packer, SimulationDomain::comm_sim_pro,
                              packer.getMPI_DataType(), send_regions,
