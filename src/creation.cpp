...
void creation::createRandom(uint32_t seed_create_types,
  ...
  lats->forAllLattices([&](const _type_lattice_coord x,
    ...
    lattice.type._type = LatticeTypes::randomAtomsType(
        types.data(), types_ratio.data(), types.size(), rand_hit);
    return true;
  });
  ...
  const unsigned long va_local =
      ...
      (SimulationDomain::comm_sim_pro.own_rank <
           ...
           ? 1
           : 0);
  const comm::_type_lattice_size max_lattice_size =
      lats->meta.box_x * lats->meta.box_y * lats->meta.box_z;
  ...
  for (unsigned long va_i = 0; va_i < va_local;) {
    ...
    Lattice &lat = lats->getLat(x + lats->meta.ghost_x, y + lats->meta.ghost_y,
                                z + lats->meta.ghost_z);
...
void creation::setGlobalId(
    LatticesList *lats_list, const comm::Region<comm::_type_lattice_coord> lbr,
  ...
  for (comm::_type_lattice_coord z = lbr.z_low; z < lbr.z_high; z++) {
    for (comm::_type_lattice_coord y = lbr.y_low; y < lbr.y_high; y++) {
      for (comm::_type_lattice_coord x = BCC_DBX * lbr.x_low;
           x < BCC_DBX * lbr.x_high; x++) {
        Lattice &lattice = lats_list->getLat(x, y, z);
        const comm::_type_lattice_coord gx =
            (x - lats_list->meta.ghost_x) + BCC_DBX * gbr.x_low;
        const comm::_type_lattice_coord gy =
            (y - lats_list->meta.ghost_y) + gbr.y_low;
        const comm::_type_lattice_coord gz =
            (z - lats_list->meta.ghost_z) + gbr.z_low;
        // convert xyz to global id.
        const comm::_type_lattice_coord gid =
            gx + BCC_DBX * phase_space[0] * (gy + gz * phase_space[1]);
        lattice.id = gid;
      }
    }
  }
