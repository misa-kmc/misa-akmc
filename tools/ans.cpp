...
void ans::analysis(LatticesList *lats_list) {
  comm::Region<_type_lattice_coord> region(
      lats_list->meta.ghost_x, lats_list->meta.ghost_y, lats_list->meta.ghost_z,
    ...
    for (_type_lattice_coord y = region.y_low; y < region.y_high; y++) {
      for (_type_lattice_coord x = region.x_low; x < region.x_high; x++) {
        ...
        if (lattice.type._type == LatticeTypes::Cu) {
          ...
          const _type_neighbour_status nei_status =
              lats_list->get1nnBoundaryStatus(x, y, z);
          ...
          for (int b = 0; b < LatticesList::MAX_NEI_BITS; b++) {
            ...
            if (((nei_status >> b) & 1) &&
                (_1nn_list[b]->type._type == LatticeTypes::Cu)) {
